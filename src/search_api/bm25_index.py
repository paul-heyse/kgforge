...
class BM25Index:
    ...
    def build_from_duckdb(cls, db_path: str) -> "BM25Index":
        ...
        try:
            ds = con.execute("SELECT parquet_root FROM datasets WHERE kind='chunks' ORDER BY created_at DESC LIMIT 1").fetchone()
            if not ds: return idx
            root = ds[0]
            rows = con.execute(f"""
    ...
    def _build(self, rows):
        ...
        for chunk_id, doc_id, section, body, title in rows:
            tf: Dict[str,float] = {}
            for t in toks(body or ""): tf[t] = tf.get(t,0.0)+1.0
            for t in toks(title or ""): tf[t] = tf.get(t,0.0)+2.0
            for t in toks(section or ""): tf[t] = tf.get(t,0.0)+1.2
            dl = sum(tf.values())
            self.docs.append(BM25Doc(chunk_id=chunk_id, doc_id=doc_id or "urn:doc:fixture", title=title or "Fixture", section=section or "", tf=tf, dl=dl))
            dl_sum += dl
    ...
    def save(self, path: str) -> None:
        ...
        with open(path, "wb") as f:
            pickle.dump({"k1":self.k1,"b":self.b,"N":self.N,"avgdl":self.avgdl,"df":self.df,"docs":self.docs}, f)
    ...
    def load(cls, path: str) -> "BM25Index":
        ...
        idx.N = d["N"]; idx.avgdl = d["avgdl"]; idx.df = d["df"]; idx.docs = d["docs"]
        return idx
