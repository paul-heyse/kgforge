...
chunks_parquet : str Path to Parquet/JSONL file with chunks. backend : str, optional Backend to use: 'lucene' or 'pure'. Defaults to 'lucene'. index_dir : str, optional Output index directory. Defaults to './\_indices/bm25'.
...
dense_vectors : str Path to the dense vector payload (JSON skeleton format). index_path : str Destination path for the serialized FAISS index. factory : str FAISS factory string describing index topology. metric : str Similarity metric identifier (`"ip"` or `"l2"`).
