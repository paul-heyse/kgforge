...
class IndexStore:
    ...
    This class provides a context manager interface for managing a persistent SQLite
    database that stores code intelligence data including symbol definitions, references,
    and file metadata. The store uses WAL (Write-Ahead Logging) mode for better
    concurrency and NORMAL synchronous mode for balanced performance and durability.

    The database schema tracks:
    - File metadata (path, language, modification time, size)
    - Symbol definitions (functions, classes, etc.)
    - Reference relationships (calls, imports, etc.)

    Parameters
    ----------
    db_path : Path
        Path to SQLite database file. The file will be created if it doesn't exist
        when the store is opened via the context manager.
    """
    ...
    def __enter__(self) -> Self:
        ...
        Opens the SQLite database connection, configures WAL mode for better
        concurrency, and sets synchronous mode to NORMAL for balanced performance.

        Returns
        -------
        Self
            Self for context manager protocol, allowing chained usage.
        """
...
def replace_file(store: IndexStore, path: Path, meta: FileMeta) -> None:
    ...
    if sym_rows:
        store.executemany(
            """INSERT INTO symbols(path, lang, kind, name, qualname, start_line,
