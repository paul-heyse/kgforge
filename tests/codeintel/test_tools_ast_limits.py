...
def test_get_ast_respects_size_limit(repo_fixture: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    # Reload tools module to use new LIMITS
...
def test_get_ast_bounded_traversal(repo_fixture: Path) -> None:
    ...
    # Reload config to ensure default limits are used
    import importlib

    import codeintel.config

    importlib.reload(codeintel.config)
    import codeintel.mcp_server.tools

    importlib.reload(codeintel.mcp_server.tools)
    # Create a file with many nested structures
    content = "def f1():\n" + "    " * 50 + "pass\n" * 100
    testfile = repo_fixture / "pkg" / "deep.py"
    testfile.write_text(content)
    result = codeintel.mcp_server.tools.get_ast("pkg/deep.py", "python", "json")
