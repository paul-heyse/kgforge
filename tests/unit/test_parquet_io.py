...
def test_parquet_dense_write(tmp_path):
    ...
    vec = [0.0]*2560
    out = w.write_dense("Qwen3-Embedding-4B","run123",2560,[("chunk:1",vec,1.0)])
...
def test_parquet_splade_write(tmp_path):
    ...
    w = ParquetVectorWriter(str(root))
    out = w.write_splade("SPLADE-v3-distilbert","run123",[("chunk:1",[1,2,3],[0.1,0.2,0.3])])
    files = list(root.rglob("*.parquet"))
...
def test_parquet_chunks_write(tmp_path):
    ...
    out = w.write([{
        "chunk_id":"urn:chunk:abc:0-10","doc_id":"urn:doc:abc","section":"Intro",
        "start_char":0,"end_char":10,"doctags_span":{"node_id":"n1","start":0,"end":10},
        "text":"Hello world","tokens":2,"created_at":0
    }])
