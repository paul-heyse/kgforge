...
def get_operation_override(
    ...
    This function retrieves operation-specific override metadata from the augment
    configuration, allowing CLI operations to customize their descriptions, help text,
    and other metadata beyond the default registry definitions.

    Parameters
    ----------
    subcommand : str
        Subcommand name (e.g., "query", "symbols") to look up override metadata.
    tokens : Sequence[str] | None, optional
        Optional token sequence for hierarchical operation lookups, by default None.

    Returns
    -------
    OperationOverrideModel | None
        Operation override metadata when present; otherwise ``None``. Overrides
        can customize operation descriptions, help text, and other metadata.
    """
