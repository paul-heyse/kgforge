...
- Introducing new documentation suites or test frameworks; verification relies on existing Ruff, pyrefly, and pyright gates.
...
1. **Explicit package surfaces:** Build each package’s `__all__` from a `PUBLIC_EXPORTS` mapping and sync it with `stubs/tools/**`, ensuring pyright and pyrefly share a consistent public API.
...
- **Breaking downstream tooling:** Mitigated by sequencing phases, running Ruff/pyrefly/pyright after each tranche, and keeping shim facades until consumers migrate.
...
7. After each phase, run `uv run ruff check --fix`, `uv run pyrefly check`, `uv run pyright --warnings --pythonversion=3.13`, and once contracts exist, run `python tools/make_importlinter.py --check`.
