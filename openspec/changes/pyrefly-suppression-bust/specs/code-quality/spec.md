...
- **WHEN** `uv run ruff format && uv run ruff check --fix`, `uv run pyrefly check`, and `uv run pyright --warnings --pythonversion=3.13 tools/_shared tools/docstring_builder tools/docs tools/navmap` execute
...
- **WHEN** `uv run ruff format && uv run ruff check --fix`, `uv run pyrefly check`, and `uv run pyright --warnings --pythonversion=3.13` cover `src/search_api/faiss_adapter.py src/vectorstore_faiss/gpu.py` in both CPU-only and GPU-extra environments
...
- **WHEN** `uv run ruff format && uv run ruff check --fix`, `uv run pyrefly check src/search_api/app.py kgfoundry_common/errors/http.py`, and `uv run pyright --warnings --pythonversion=3.13 src/search_api/app.py kgfoundry_common/errors/http.py` execute
