...
- [x] **1.3 FastAPI & settings typing**
  ...
  - [x] Execute `uv run ruff format && uv run ruff check --fix`, `uv run pyrefly check src/search_api/app.py kgfoundry_common/errors/http.py`, and `uv run mypy --config-file mypy.ini src/search_api kgfoundry_common` after each milestone to ensure suppressions are not reintroduced (principles 4, 5).
...
- [ ] **1.4 Agent catalog & NumPy helpers**
  ...
  - [ ] 1.4.5 Run `uv run ruff format && uv run ruff check --fix`, `uv run pyrefly check src/kgfoundry/agent_catalog`, and `uv run mypy --config-file mypy.ini src/kgfoundry/agent_catalog` to certify the refactor; capture quick latency/memory measurements in a developer note (principles 4, 9, 12).
...
- [x] 2.3 Run `uv run mypy --config-file mypy.ini` for affected packages (`tools`, `src/search_api`, `src/vectorstore_faiss`, `src/kgfoundry_common`).
...
- [x] 4.2 Monitor CI (pyrefly/mypy) for regressions after merge; open follow-up tickets for any third-party stub contributions upstream.
